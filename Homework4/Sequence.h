...
{
public:
	Sequence() { createEmpty();	}			     // Create an empty sequence (i.e., one whose size() is 0).
	bool empty() const { return size() == 0; }  // Return true if the sequence is empty, otherwise false.
	int size() const { return m_size; }        // Return the number of items in the sequence.

...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	...
	for (int i = 1; i < m_size; i++) {
		int location = i;
		while (location > 0 && !comp(s[location - 1], s[location])) {
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Sensor*> ptrSensors;// TODO:  Create a vector of Sensor pointers, and set each pointer
									//        to point to the corresponding Sensor in auxSensors.
		for (size_t i = 0; i < auxSensors.size(); i++) {
			ptrSensors.push_back(&auxSensors[i]);
		}
